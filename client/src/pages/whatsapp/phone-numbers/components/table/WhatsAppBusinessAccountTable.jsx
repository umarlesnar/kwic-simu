...
const QualityUpdateButtonGroup = ({ wba_id, item }) => {
  const handleBtnNavigation = useCallback(
    async (type) => {
      if (type === "ERROR_MESSAGE") {
        const wbMessages = new WBMessages(item.wba_id, item.id);
        wbMessages.display_phone_number = item.display_phone_number;
        const response = await WebhookService.push(
          wbMessages.getErrorMessage()
        );
        console.log("RESPONE", response);
      } else {
        const phoneNumber = new WBPhoneNumberStatus(item.wba_id);
  ...
  return (
    <div className="mt-1 flex justify-start gap-2">
      ...
      <span
        onClick={() => handleBtnNavigation("ERROR_MESSAGE", item)}
        className="inline-flex items-center rounded-md bg-blue-50 px-2 py-1 text-xs font-medium text-blue-700 ring-1 ring-blue-700/10 ring-inset cursor-pointer"
      >
        ERROR_MESSAGE
      </span>
...
const WhatsAppBusinessAccountTable = () => {
  ...
  const handleBtnNavigation = useCallback((type, item) => {
    if (type == "CLIENTS") {
      navigate(
        `/whatsapp/client/${item.wba_id}/${item.id}?catalog_id=${item.catalog_id}`,
        {
      ...
      navigate(
        `/whatsapp/ecommerce/${item.wba_id}/${item.id}?display_phone_number=${item.display_phone_number}`,
        {
          state: { returnPath: "/whatsapp" },
        }
      );
  ...
  const AccountUpdateButtonGroup = ({ wba_id }) => {
      ...
      async (type) => {
        ...
        if (type === "VERIFICATION") {
          await WebhookService.push(
            webhook_payload.getBusinessVerificationStatus()
          );
        } else if (type === "VIOLATION") {
          await WebhookService.push(webhook_payload.getAccountViolation());
        }
    ...
    return (
      <div className="relative">
        <span
          ...
          className="inline-flex items-center rounded-md bg-red-50 px-2 py-1 text-xs font-medium text-red-700 ring-1 ring-red-700/10 ring-inset cursor-pointer"
        >
          Status
        </span>
        {showDropdown && (
          <div className="absolute top-full left-0 mt-1 bg-white border border-gray-300 rounded shadow-lg z-10 min-w-40">
            <div
              onClick={() => handleAccountUpdate("VERIFICATION")}
              className="px-3 py-2 text-sm hover:bg-gray-100 cursor-pointer text-gray-500 flex gap-1 items-center"
            >
              <MdAccountBox />
              Business Rejected
            </div>
            <div
              onClick={() => handleAccountUpdate("VIOLATION")}
              className="px-3 py-2 text-sm hover:bg-gray-100 cursor-pointer text-gray-500 flex gap-1 items-center"
            >
              <IoRemoveCircle /> Spam
            </div>
                ...
                <td className="px-3 py-2 whitespace-nowrap">
                  ...
                  <div className="mt-1 space-x-2">
                    ...
                    <div className="inline-flex">
                      <AccountUpdateButtonGroup wba_id={business.wba_id} />
                    </div>
                  </div>
                </td>
                <td className="px-3 py-2 whitespace-nowrap">
                  <div className="text-sm text-gray-500">
                    ...
                    <div className="mt-1 flex justify-start gap-2">
                      ...
                      <span
                        onClick={() =>
                          handleBtnNavigation("ECOMMERCE", business)
                        }
