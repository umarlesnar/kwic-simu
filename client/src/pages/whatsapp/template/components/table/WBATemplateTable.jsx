...
const CategoryDropdown = ({ template, wba_id }) => {
  ...
  const handleCategoryChange = useCallback(
    async (newCategory) => {
      ...
      webhook_payload.message_template_name = template.data.name;
      webhook_payload.message_template_language = template.data.language;
      await WebhookService.push(webhook_payload.getTemplateCategoryChange(newCategory));
  ...
  return (
    <div>
      <div className="mb-2">{template.category}</div>
      <div className="relative">
        <button
          onClick={() => setShowDropdown(!showDropdown)}
          className="inline-flex items-center rounded-md bg-purple-50 px-2 py-1 text-xs font-medium text-purple-700 ring-1 ring-purple-700/10 ring-inset cursor-pointer"
        >
          Change Category
        </button>
        {showDropdown && (
...
const WBATemplateTable = ({ wba_id }) => {
  ...
  const columns = React.useMemo(
    () => [
      ...
      { Header: "Category", Cell: ({ row }) => <CategoryDropdown template={row.original} wba_id={wba_id} /> },
