class WBMessageStatus {
  ...
  getObject() {
    ...
    let statusPayload = {};
    if (this.type === "sent") {
      // Only include conversation when it's available and has expected fields
      const conv = this.conversation && this.conversation.id ? {
        ...
        expiration_timestamp: this.conversation.expiration_timestamp
          ? this.conversation.expiration_timestamp.toFixed(0).toString()
          : undefined,
      } : undefined;
      ...
      // Guard conversation as with 'sent' - only include when present
      const convDelivered = this.conversation && this.conversation.id ? {
        id: this.conversation.id,
        origin: this.conversation.origin,
        pricing: this.conversation.pricing,
      } : undefined;
      ...
      statusPayload = {
        ...
        timestamp: (Date.now() / 1000).toFixed(0).toString(),
        recipient_id: this.wa_id,
        ...(convDelivered ? { conversation: convDelivered } : {}),
      };
