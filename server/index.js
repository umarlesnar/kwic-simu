...
const { OperatorStatusBatcher } = require("./utils/OperatorStatusBatcher");
...
const operatorStatusBatcher = process.env.OPERATOR_WEBHOOK_URL
  ? new OperatorStatusBatcher(process.env.OPERATOR_WEBHOOK_URL)
  : null;
...
io.use(async (socket, next) => {
  const roomId = socket.handshake.query.ppk || undefined;
  ...
  if (token) {
    ...
    if (!decoded) {
      ...
      if (decoded.sub) {
        ...
        user_payload["id"] = decoded.sub;
        user_payload["session_id"] = decoded?.k_session_id || "";
      }
      // Attach user info to socket
      socket.user = user_payload;
    }
  } else {
    // Attach user info to socket
    socket.user = user_payload;
...
io.on("connection", async (socket) => {
  ...
  if (operatorStatusBatcher && socket.user?.session_id) {
    setImmediate(() => {
      operatorStatusBatcher.addStatusUpdate(socket.user.session_id, true);
    });
  }
  ...
  socket.on("disconnect", async () => {
    ...
    if (operatorStatusBatcher && socket.user?.session_id) {
      setImmediate(() => {
        operatorStatusBatcher.addStatusUpdate(socket.user.session_id, false);
      });
    }
  });
});
