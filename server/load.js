...
const wba_account = [
  ...
  },
  {
    ...
    phone_numbers: [
      ...
      {
        id: "120000035",
        display_phone_number: "+91 9000000035",
        verified_name: "QA Alpha Store",
        code_verification_status: "VERIFIED",
        quality_rating: "GREEN",
        platform_type: "CLOUD_API",
        throughput: { level: "STANDARD" },
      ...
      {
        id: "120000036",
        display_phone_number: "+91 9000000036",
        verified_name: "QA Beta Group",
        code_verification_status: "VERIFIED",
        quality_rating: "GREEN",
        platform_type: "CLOUD_API",
        throughput: { level: "STANDARD" },
...
const business_account_catelogs = [
  {
    id: "1600000001",
    catalogs: [
      {
...
async function loadMockData() {
  try {
    ...
    for (const app of wba_account) {
      ...
      const redisKey = `whatsapp-account:${app.wba_id}`;
      await redis.set(redisKey, JSON.stringify(app.meta_data));
    }

    for (const bc of business_account_catelogs) {
      const redisKey = `business-account-catalogs:${bc.id}`;
      await redis.set(redisKey, JSON.stringify(bc.catalogs));
    }
